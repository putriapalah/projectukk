        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('suratUkur.update', $suratUkur->kodeBT) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select id="jenis_hak" name="jenis_hak" class="form-control" required>
                                ...
                                @foreach ($options as $opt)
                                    <option value="{{ $opt }}"
                                        {{ old('jenis_hak', $suratUkur->jenis_hak) === $opt ? 'selected' : '' }}>
                                        {{ $opt }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input id="lokasi_penyimpanan" name="lokasi_penyimpanan" class="form-control"
                                value="{{ old('lokasi_penyimpanan', $suratUkur->lokasi_penyimpanan) }}">
                        </div>

                        <button class="btn btn-primary" type="submit">Update</button>
                        <a href="{{ route('suratUkur.index') }}" class="btn btn-secondary">Kembali</a>
                    </form>
                </div>
            </div>
        </div>
